...
execute if predicate siscu:utils/10_percent if predicate siscu:locations/check_metamorphic_caves if block ~ ~-1 ~ #siscu:fire_spring_blocks unless entity @e[tag=fire_spring,distance=..16] if score fire_springs_enabled siscu.integer matches 1 run tag @s add fire_spring
