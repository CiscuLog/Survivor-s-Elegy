$summon wither_skeleton ~ ~ ~ {Tags:["siscu.entity_checked","siscu.parched_checked","siscu.converted","siscu.withered"],Rotation:$(Rotation),fall_distance:$(fall_distance),\
\
...
execute unless items entity @s armor.head * run item replace entity @n[tag=siscu.converted] armor.head with sand[item_model="siscu_se:parched_headgear"]

data modify entity @n[tag=siscu.converted] Tags append from entity @s Tags[]


execute if entity @s[tag=siscu.baby] as @n[tag=siscu.converted] run function siscu:entities/parched/summon_wither_baby
