...
execute if entity @s[tag=zombifying_invulnerable] run return 0

execute unless predicate siscu:entities/has_phage run function siscu:entities/zombifying/apply_phage

execute if entity @s[type=!#siscu:zombie_compatible,tag=!infected_zombie] run return run damage @s 1 siscu:phage
