...
## Piglin skeletons
execute at @s if predicate siscu:utils/75_percent if predicate siscu:utils/check_the_nether run tag @s[tag=!husk_skeleton,tag=!villager] add piglin
execute if entity @s[tag=piglin] run function siscu:entities/skeleton/piglin

## Villager Skeletons
execute at @s if predicate siscu:utils/5_percent if predicate siscu:utils/check_overworld run tag @s[tag=!husk_skeleton] add villager
...
## Husk Skeletons
execute at @s[tag=!villager] if biome ~ ~ ~ minecraft:desert if predicate siscu:locations/check_surface if predicate siscu:utils/80_percent run tag @s add husk_skeleton
