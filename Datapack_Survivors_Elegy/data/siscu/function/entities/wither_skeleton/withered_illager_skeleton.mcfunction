
loot replace entity @s weapon loot siscu:entities/wither_skeleton/wither_skeleton_weapons
loot replace entity @s weapon.offhand loot siscu:entities/wither_skeleton/wither_skeleton_illager_shields

loot replace entity @s armor.head loot siscu:entities/wither_skeleton/wither_skeleton_illager_skulls
loot replace entity @s armor.chest loot siscu:entities/wither_skeleton/wither_skeleton_chestplates
loot replace entity @s armor.legs loot siscu:entities/wither_skeleton/wither_skeleton_leggings
loot replace entity @s armor.feet loot siscu:entities/wither_skeleton/wither_skeleton_boots


data modify entity @s ArmorDropChances[3] set value 0.0f