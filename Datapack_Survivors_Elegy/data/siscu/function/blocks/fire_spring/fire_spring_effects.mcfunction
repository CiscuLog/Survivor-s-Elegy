...
execute unless block ~ ~-1 ~ #siscu:fire_spring_blocks run return run function siscu:blocks/fire_spring/fire_spring_explode
execute if score @s siscu.fire_spring >= fire_spring_limit siscu.integer run return run function siscu:blocks/fire_spring/fire_spring_burst
...
execute if score @s siscu.fire_spring >= fire_spring_sound siscu.integer if predicate siscu:utils/25_percent run playsound minecraft:block.lava.pop ambient @a ~ ~ ~
return 1