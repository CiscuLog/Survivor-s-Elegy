...
execute as @e[tag=fire_spring_spawned] run data merge entity @s {item:{id:"minecraft:bone",count:1b,components:{"item_model":"siscu_se:block/fire_spring"}},transformation:{scale:[1.01,1.01,1.01],translation:[0.0,-0.5,0.0]},start_interpolation: 0}
...
execute as @e[tag=fire_spring] unless score @s siscu.fire_spring matches 0.. run function siscu:blocks/fire_spring/set_name with storage siscu:volatile
