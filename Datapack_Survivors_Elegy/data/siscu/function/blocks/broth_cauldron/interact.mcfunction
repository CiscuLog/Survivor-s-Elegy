...
# Broth potions. Removed in v1.0.0, to be added back in the future
...
# (get broth data)
execute at @s as @n[type=marker,tag=siscu.broth_interacting] at @s run function siscu:blocks/broth_cauldron/update/get_data
# available = check_available() --> check if item can be added based on state and broth level
execute at @e[type=marker,tag=siscu.broth_interacting] store result score available siscu.broth_data run function siscu:blocks/broth_cauldron/check_availability
...
execute as @e[type=marker,tag=siscu.broth_interacting] at @s run function siscu:blocks/broth_cauldron/update/textures
