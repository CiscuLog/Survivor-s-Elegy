...
execute at @s as @e[type=!#siscu:discharge_immune,tag=!discharging,distance=..10] at @s anchored eyes run summon area_effect_cloud ~ ~1 ~ {Tags:["siscu.discharge_path"],Duration:1,Radius:0,Age:20}
## Face AEC towards the player and make it move
execute as @e[type=area_effect_cloud,tag=siscu.discharge_path] at @s facing entity @p[tag=discharging] feet run function siscu:items/passive_behav/discharge/effect
