...
# Safe Mode
execute if score safe_mode siscu.integer matches 1 run tellraw @s [{"text":"\u26A0 Server Mode Activated \u26A0\n","bold":true},{"text":"This is an automatic feature and cannot be deactivated ","bold": false}, {"text": "[Learn More]","hover_event": {"action": "show_text","value": "Click"},"click_event": {"action": "run_command","command": "/function siscu:technical/config/about_safe_mode"}}]
# Menu
